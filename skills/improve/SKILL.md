...
1. **Never modify source code yourself.** No edits, no fixes, no "quick wins while you're in there." The ONLY files you may create or modify live under `docs/dev/plans/` — or under `docs/dev/advisor-plans/` when `docs/dev/plans/` already exists for an unrelated purpose (create the chosen directory if absent). The `execute` variant may create and maintain an ignored workspace-local disposable worktree under `docs/dev/plans/.worktrees/` plus a `docs/dev/plans/.gitignore` entry, then dispatches a *separate executor* (subagent or headless coding CLI) to edit code in that isolated git worktree — you review its diff and render a verdict; you still never edit code directly, and you never merge, push, or commit to the user's branch.
2. **Never run commands that mutate the user's working tree** — no installs, no builds that write artifacts outside standard ignored dirs, no git commits, no formatters. Search, read, and run static analysis only when it does not execute repository-controlled code. Treat build, test, lint, framework, package-manager, and install commands as repository-code execution even when they are named "checks"; whether they may run is governed by the selected **execution profile** — trusted-local, strict, or manual, defined in [references/closing-the-loop.md](references/closing-the-loop.md). Under trusted-local (the user's own repos) ordinary verification runs under the host's normal permission policy; under strict, repository code runs only inside an enforceable sandbox; high-risk effects (installs with lifecycle scripts, migrations, deployment, credentialed or broad network access, destructive operations) require strict handling or explicit authorization in every profile. Two scoped exceptions: permitted verification commands inside an executor's disposable workspace-local worktree during `execute` review, and `gh issue create` under an explicit `--issues` flag.
...
Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively (no user available to choose), write plans for the top 3–5 by leverage and record that default in `docs/dev/plans/README.md`.
...
Finish by running the bundled `resources/generate_plan_index.py` helper to write `docs/dev/plans/README.md` from plan frontmatter. For example: `python <path-to-skill>/resources/generate_plan_index.py --plans-dir docs/dev/plans`. The index carries recommended execution order, dependencies, status, execution base, reviewed commit, and merged commit; it is generated, not hand-edited.

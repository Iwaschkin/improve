...
- the compact causal contract, inlined: corrective findings return a causal status (`CONFIRMED` with the four-link chain — input/condition → code path → specific flaw → symptom/impact — or `HYPOTHESIS`), the observed condition, the correct fix layer, and the likely symptom-level shortcuts to reject. Workers return evidence and causal status only; they never implement a fix.
